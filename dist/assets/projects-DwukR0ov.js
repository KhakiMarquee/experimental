import"./ui-kBzOPii5.js";import"./category-0rqBLcvf.js";function p(e,t){e.classList.add("expanding");const o=e.getBoundingClientRect(),d=o.top+window.scrollY,r=o.left+window.scrollX,a=o.width,l=o.height;e.style.transition="none",e.style.position="fixed",e.style.top=`${d}px`,e.style.left=`${r}px`,e.style.width=`${a}px`,e.style.height=`${l}px`,e.style.margin="0",e.style.transform="none",e.style.zIndex="10000",e.style.willChange="top,left,width,height,transform",document.body.appendChild(e),e.getBoundingClientRect();const n=600;e.style.transition=`top ${n}ms ease, left ${n}ms ease, width ${n}ms ease, height ${n}ms ease, transform ${n}ms ease`,e.style.top="0px",e.style.left="0px",e.style.width="100vw",e.style.height="100vh",e.classList.add("fullscreen");const i=s=>{s.target===e&&e.classList.contains("fullscreen")&&(e.removeEventListener("transitionend",i),e.style.willChange="",c(t))};e.addEventListener("transitionend",i),setTimeout(()=>{document.body.contains(e)&&c(t)},n+50)}function c(e){document.body.innerHTML=`
<div id="detail-app">
  ...
  `,document.getElementById("close-detail").addEventListener("click",()=>{location.reload()})}function m(){return new URLSearchParams(window.location.search).get("category")}function g(e,t){const o=document.getElementById("content"),d=document.getElementById("page-title"),r=document.getElementById("project-section-description");fetch(t).then(a=>{if(!a.ok)throw new Error("Failed to fetch data.");return a.json()}).then(a=>{const l=a[e];if(!l)throw new Error(`Category "${e}" not found in data.`);d.textContent=e.charAt(0).toUpperCase()+e.slice(1),r&&(r.textContent=l.description||""),o.innerHTML="";const n=l.items;n&&n.length?n.forEach(i=>{const s=document.createElement("div");s.addEventListener("click",()=>{p(s,i)}),s.classList.add("project-row"),s.innerHTML=`
          ...
          `,o.appendChild(s)}):o.innerHTML=`<p>No content found for "${e}".</p>`}).catch(a=>{console.error("Error:",a),o.innerHTML="<p>Error loading content.</p>"})}document.addEventListener("DOMContentLoaded",()=>{const e=m();e?g(e,"/data/data.json"):document.getElementById("content").innerHTML="<p>Invalid category.</p>",document.getElementById("grid-view").addEventListener("click",()=>{const t=document.querySelector(".projects-container");t.classList.add("row-view"),t.classList.remove("list-view"),document.getElementById("grid-view").classList.add("active"),document.getElementById("list-view").classList.remove("active")}),document.getElementById("list-view").addEventListener("click",()=>{const t=document.querySelector(".projects-container");t.classList.add("list-view"),t.classList.remove("row-view"),document.getElementById("list-view").classList.add("active"),document.getElementById("grid-view").classList.remove("active")})});
