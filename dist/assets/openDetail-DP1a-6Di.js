function g(t,e){const r=`${window.location.origin}${window.location.pathname}?id=${e.id}&title=${encodeURIComponent(e.title)}`;window.history.replaceState({itemTitle:e.title},e.title,r),t.classList.add("expanding"),window.p5Instance&&window.p5Instance.noLoop();const o=t.getBoundingClientRect(),s=o.top+window.scrollY,l=o.left+window.scrollX,p=o.width,c=o.height;t.parentElement,t.nextSibling,t.style.transition="none",t.style.position="fixed",t.style.top=`${s}px`,t.style.left=`${l}px`,t.style.width=`${p}px`,t.style.height=`${c}px`,t.style.margin="0",t.style.transform="none",t.style.zIndex="10000",t.style.willChange="top,left,width,height,transform",document.body.appendChild(t),t.getBoundingClientRect();const n=600;t.style.transition=`top ${n}ms ease, left ${n}ms ease, width ${n}ms ease, height ${n}ms ease, transform ${n}ms ease`,t.style.top="0px",t.style.left="0px",t.style.width="100vw",t.style.height="100vh",t.classList.add("fullscreen");const a=d=>{d.target===t&&t.classList.contains("fullscreen")&&(t.removeEventListener("transitionend",a),t.style.willChange="",i(e))};t.addEventListener("transitionend",a),setTimeout(()=>{document.body.contains(t)&&i(e)},n+50)}function i(t){document.body.innerHTML=`
    <div id="detail-app">
      ...
      <main style="max-width:960px; width:100%; margin:0 auto;">
        ...
        ${t.metadata?.question?`<p style="margin:0 0 8px 0;"><strong>Question:</strong> ${t.metadata.question}</p>`:""}

        ${t.metadata?.process?`<p style="margin:0 0 8px 0;"><strong>Process:</strong> ${t.metadata.process}</p>`:""}
